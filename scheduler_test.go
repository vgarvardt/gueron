...
func TestNewScheduler(t *testing.T) {
	...
	s, err := NewScheduler(nil, WithHorizon(10*time.Minute), WithLogger(logger))
...
func Test_schedulesHash(t *testing.T) {
	...
	s4, err := NewScheduler(nil, WithQueueName(randomQueueName))
	require.NoError(t, err)
...
func Test_scheduleJobs(t *testing.T) {
	...
	rows, err := pool.QueryContext(ctx, `SELECT job_type, run_at FROM gue_jobs WHERE queue = $1 ORDER BY run_at ASC`, queue)
	require.NoError(t, err)
...
func Test_refreshSchedule(t *testing.T) {
	...
	_, err := pool.ExecContext(ctx, `TRUNCATE gueron_meta`)
	require.NoError(t, err)
	...
	err = pool.
		QueryRowContext(ctx, `SELECT queue, hash, scheduled_at, horizon_at FROM gueron_meta`).
